...
func TestGuard_APIKeyValid(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			db:       &DBMock{ExpAPIKsBUsrIDValErr: errors.NewNotFound("")},
			expErr:   false,
		},
		{
			name:            "empty",
		...
		},
		{
			...
			key:             validKey.Value(),
			expUsrID:        validUsrID,
			db:              &DBMock{ExpAPIKsBUsrIDValErr: errors.NewNotFound("no keys for 12345")},
			expErr:          true,
			expForbidden:    true,
			expUnauthorized: false,
		},
		{
			...
			key:      validKey.Value(),
			expUsrID: validUsrID,
			db:       &DBMock{ExpAPIKsBUsrIDValErr: errors.New("some errors")},
			expErr:   true,
		},
	}
	for _, tc := range tt {
